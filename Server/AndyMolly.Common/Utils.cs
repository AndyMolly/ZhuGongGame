...
namespace AndyMolly.Common
{
   public class Utils
   {
       ...
       /// <param name="linkUrl">链接地址，__id__代表页码</param>
       /// <param name="centSize">中间页码数量</param>
       /// <param name="showConten">显示数据条数标记</param>
       /// <returns></returns>
       public static string OutPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize, string showConten)
       {
           //计算页数
           if (totalCount < 1 || pageSize < 1)
           {
           ...
           StringBuilder pageStr = new StringBuilder();
           string pageId = "__id__";
           string firstBtn = "<li><a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex - 1).ToString()) + "\"><i class=\"icon icon-angle-left\"></i></a></li>";
           string lastBtn = "<li><a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex + 1).ToString()) + "\"><i class=\"icon icon-angle-right\"></i></a></li>";
           ...
           if (pageIndex == 1)
           {
               firstStr = "<li class=\"active\"><a>1</a></li>";
           }
           if (pageIndex == pageCount)
           {
               lastStr = "<li class=\"active\"><a>" + pageCount.ToString() + "</a></li>";
           }
           int firstNum = pageIndex - (centSize / 2); //中间开始的页码
           if (pageIndex < centSize)
               firstNum = 2;
           ...
           pageStr.Append(" <div class=\"total\">" + totalCount + showConten + "</div>");
           pageStr.Append("<div class=\"pagination-warp\"><ul class=\"pagination\">");
           pageStr.Append(firstBtn + firstStr);
           if (pageIndex >= centSize)
           {
               pageStr.Append("<span>...</span>\n");
           ...
           for (int i = firstNum; i <= lastNum; i++)
           {
               if (i == pageIndex)
               {
                   pageStr.Append("<li class=\"active\"><a href=\"" + ReplaceStr(linkUrl, pageId, i.ToString()) + "\">" + i + "</a></li>");
               }
               else
               {
           ...
           pageStr.Append("</ui>");
           //开始跳转框
           pageStr.Append("<div class=\"page-skip\" id=\"page-skip\">");
           pageStr.Append("<div class=\"input-group\">");
           pageStr.Append("<input type=\"text\" class=\"form-control\" value=\"\" name=\"page\">");
           pageStr.Append("<span class=\"input-group-btn\">");
           pageStr.Append("<button type=\"submit\" class=\"btn\">跳转</button>");
           pageStr.Append("</span></div></div>");
           //跳转结束
           pageStr.Append("</div>");
           return pageStr.ToString();
       }
